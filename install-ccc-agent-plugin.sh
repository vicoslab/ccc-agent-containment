...
#   ~/.local/bin/               — unified CLI shim: ccc-agent
...
do_it cp "$SCRIPT_DIR/bin/ccc-agent" "$PREFIX/bin/ccc-agent"
do_it chmod +x "$PREFIX/bin/ccc-agent"
...
export CCC_AGENT_BRANCHFS_BIN="${BRANCHFS_BIN:-}"
exec "$PREFIX/bin/ccc-agent" "\$@"
WRAPPER_EOF
    chmod +x "$WRAPPER"
else
